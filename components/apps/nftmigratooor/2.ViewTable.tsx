...
const ViewTableOpenSea = memo(function ViewTableOpenSea({onProceed}: {onProceed: VoidFunction}): ReactElement {
	 ...
	 **********************************************************************************************/
	const selectedPerCollection = useMemo(
		(): TDict<TNFT[]> =>
			(nfts || []).reduce((acc: TDict<TNFT[]>, obj: TNFT): TDict<TNFT[]> => {
				let key = obj.collection.address as string;
				if (isLensNFT(obj.collection.name)) {
	...
	const onSelectAll = useCallback(
		(areAllChecked: boolean, value: TNFT[]): void => {
			if (areAllChecked) {
	...
	const isItemSelected = useCallback(
		(nft: TNFT): boolean => selected.find((asset: TNFT): boolean => asset.id === nft.id) !== undefined,
		[selected]
	);
	...
	const isCollectionSelected = useCallback(
		(key: string, value: TNFT[]): boolean => selectedPerCollection?.[key]?.length === value?.length,
				...
				<div className={'grid gap-0 pt-4'}>
					{isFetchingNFTs ? (
						...
						Object.entries(groupedByCollection).map(
							([key, value]: [string, TNFT[]]): ReactElement => (
								<NFTCollection
									key={key}
